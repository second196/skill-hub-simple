...
async function confirmAccountStatus(): Promise<void> {
  if (!accountTarget.value) return
  accountBusy.value = true
  try {
    await updateAccountStatus(accountTarget.value.username, !accountTarget.value.enabled)
    accountTarget.value.enabled = !accountTarget.value.enabled
    showToast(accountTarget.value.enabled ? '账户已启用' : '账户已停用', { description: accountTarget.value.username, tone: 'success' })
    accountTarget.value = null
  } catch (reason: unknown) {
    error.value = reason instanceof Error ? reason.message : '账户状态更新失败'
  } finally {
...
async function addMember(): Promise<void> {
  ...
  try { await addNamespaceMember(selectedNamespace.value, memberUsername.value.trim(), memberRole.value); showToast('成员已添加', { description: memberUsername.value.trim(), tone: 'success' }); memberUsername.value = ''; await loadMembers() }
...
<template>
  <main class="page-shell">
    ...
    <section v-if="activeTab === 'users'" class="content-section"><div class="section-heading"><div><p class="section-kicker">账户</p><h2>用户管理</h2><p class="muted">启用或停用平台登录账户。</p></div><span class="result-count">{{ accounts.length }} 个账户</span></div><SkeletonLoader v-if="loading" :rows="4" /><EmptyState v-else-if="accounts.length === 0" compact title="暂无用户" /><div v-else class="table-wrap"><table><thead><tr><th>用户</th><th>用户 ID</th><th>状态</th><th>操作</th></tr></thead><tbody><tr v-for="account in accounts" :key="account.id"><td><span class="admin-user-cell"><b>{{ account.username.slice(0, 1).toUpperCase() }}</b><strong>{{ account.username }}</strong></span></td><td><code>{{ account.id }}</code></td><td><span class="status" :class="account.enabled ? 'published' : 'blocked'">{{ account.enabled ? '已启用' : '已停用' }}</span></td><td><button :class="account.enabled ? 'button-danger' : 'button-secondary'" type="button" @click="accountTarget = account">{{ account.enabled ? '停用' : '启用' }}</button></td></tr></tbody></table></div></section>
    ...
    <ModalDialog :open="Boolean(accountTarget)" :title="accountTarget?.enabled ? '确认停用账户' : '确认启用账户'" :description="accountTarget ? `${accountTarget.enabled ? '停用' : '启用'}账户“${accountTarget.username}”后，${accountTarget.enabled ? '该账户将无法继续登录平台' : '该账户将恢复登录权限'}。` : ''" :confirm-text="accountTarget?.enabled ? '停用账户' : '启用账户'" :danger="accountTarget?.enabled" :busy="accountBusy" @close="accountTarget = null" @confirm="confirmAccountStatus" />
  </main>
</template>
