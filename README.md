...
| 选项 | 默认值 | 说明 |
| --- | --- | --- |
| `--category <category>` | 无 | 技能分类；**包内 category 优先**，平台已有 skill 继承原分类，都没有时才用本参数 |
...
| `--json` | 关闭 | 输出 JSON |
| `--no-check` | check 开启 | 跳过上传前本地包完整性检查 |

示例：

```bash
skillhub upload ./my-skill --json
skillhub upload ./my-composite --category 研发 --json
skillhub upload ./my-skill.zip --service-url http://192.168.1.10:8080
skillhub check ./my-skill --json
...
- **临时目录 write-back 强制**：temp 补全元数据后必须 `skillhub prepare <源目录> --complete-from <temp>` 覆盖回技能源目录并删除 temp，再 `verify-source` + 对**源目录** upload。禁止「仅临时目录成功、源目录仍缺 package.json」。
- **发布成功标准**：平台 `list` 核验通过 **且** `verify-source` 源目录通过。
...
- 没有根目录 `SKILL.md` 但包含多个嵌套 `SKILL.md` 时，识别为复合 Skill；**不会自动生成根 `SKILL.md`**。
...
| `--skill-version <semver>` | 最新版本 | 安装指定包内 SemVer `version_label`（不要用全局 `--version`） |
| `--service-url <url>` | `http://127.0.0.1:8080` | SkillHub 后端地址 |
| `--json` | 关闭 | 输出 JSON |
...
skillhub install using-product-development --skill-version 1.0.1
