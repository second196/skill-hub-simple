...
export async function drainQueue(options = {}) {
    ...
    try {
        ...
        if (options.reconcile)
            await reconcileSources(sourceCache);
...
async function prepareJob(job, sourceCache) {
...
async function scanSessionEvents(clientName, sessionId, sourcePath) {
    const scanned = await scanSessionFile(clientName, sourcePath);
    return scanned.filter((event) => event.client_name === clientName && event.session_id === sessionId);
}
async function bumpJob(job, message) {
...
export function spawnDrain(args = []) {
