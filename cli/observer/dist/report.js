...
export function renderReport(events, platform, compared) {
...
function stepTitle(step) {
    ...
    if (step.type === 'assistant')
        return '助手回复';
    if (step.type === 'skill')
        return String(step.payload.name || step.skill_name || step.skill_slug || 'Skill');
    if (step.type === 'document')
        return String(step.payload.path || '文档');
    return String(step.payload.name || '工具');
}
function stepBody(step) {
    if (step.type === 'user' || step.type === 'assistant')
        return String(step.payload.text || '');
    if (step.type === 'document')
        return String(step.payload.content || '');
    try {
