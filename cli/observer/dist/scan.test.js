...
test('attributes project skill file loads, slash commands, and multi-skill sessions', async () => {
    ...
    try {
        ...
        const uploadSlugs = (sessionId) => new Set(platformOnly
...
test('does not match neighboring session ids by substring', () => {
    ...
    assert.equal(sessionMatches('parent/subagents/child', 'parent/subagents/child', 'C:\\Users\\x\\parent\\subagents\\child.jsonl', 'claude-code'), true);
    assert.equal(sessionMatches('child', 'parent/subagents/child', 'C:\\Users\\x\\parent\\subagents\\child.jsonl', 'claude-code'), false);
});
