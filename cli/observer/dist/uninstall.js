...
import { stripOwnedHooks } from './install.js';
...
export async function uninstallObserver(options = {}) {
    const lines = [];
...
async function removeLaunchers() {
...
async function stripHooksFromJson(path) {
    let text = '';
    try {
    ...
    for (const event of Object.keys(hookMap)) {
        ...
        if (!Array.isArray(entries))
            continue;
        const next = stripOwnedHooks(entries);
...
async function disableCodexFeatureHooks() {
    ...
    return new Promise((resolve, reject) => {
        ...
        child.on('close', (code) => {
            if (code === 0)
                ...
                reject(new Error(stderr.trim() || stdout.trim() || `${command} 退出码 ${code}`));
        });
    });
}
