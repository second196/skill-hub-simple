...
export function truncateText(value, maxBytes = MAX_DOCUMENT_BYTES) {
    if (value.length <= maxBytes)
        return value;
    return `${value.slice(0, maxBytes)}\n\n[document truncated; original ${value.length} chars]`;
}
export function extractPaths(value, found = new Set()) {
    if (typeof value === 'string') {
        const matches = value.match(/(?:^|[\s'"=])((?:~|\/|\.\/|\.\.\/|[A-Za-z]:[\\/])[^\s'"]+)/g);
        if (matches) {
            for (const match of matches) {
                const path = match.replace(/^[\s'"=]+/, '').replace(/[)'"]+$/, '');
                if (isDocumentPath(path) || /SKILL\.md$/i.test(path))
                    found.add(path);
            }
        }
        if (isDocumentPath(value) || /SKILL\.md$/i.test(value))
            found.add(value);
    }
...
export function valueContains(value, needle) {
    if (!needle)
        return false;
    if (typeof value === 'string') {
    ...
    if (value && typeof value === 'object')
        return Object.values(value).some((item) => valueContains(item, needle));
    return false;
}
