...
export function resolveServiceUrl(value) {
    const fromEnv = normalizeServiceUrl(process.env.SKILLHUB_SERVICE_URL);
    if (fromEnv)
        return fromEnv;
    const normalized = normalizeServiceUrl(value);
    if (!normalized)
        throw new Error('serviceUrl is empty; run skillhub-observer config set --host <ip> --port <port>');
    return normalized;
}
...
export function parseServiceUrl(value) {
...
export function describeConfig(config, path = configPath()) {
    return [
        `配置文件: ${path}`,
