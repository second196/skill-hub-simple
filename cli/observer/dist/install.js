...
async function installClaude() {
    ...
    const command = (phase) => selfCommand(['hook', '--phase', phase, '--provider', 'claude-code']);
...
async function installCodex() {
    ...
    const hooks = asRecord(settings.hooks);
    const command = (phase) => selfCommand(['hook', '--phase', phase, '--provider', 'codex']);
...
function addHook(hooks, event, command, matcher) {
