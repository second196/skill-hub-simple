...
export async function runHook(phase, provider) {
    try {
        const clientName = provider === 'codex' ? 'codex' : 'claude-code';
...
async function enqueueFromHook(clientName, payload, ended) {
...
async function readPayload() {
    ...
    try {
        for await (const chunk of process.stdin) {
            raw += chunk;
            if (Buffer.byteLength(raw) > STDIN_MAX_BYTES)
                break;
        }
    }
    catch {
