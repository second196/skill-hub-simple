...
cli.command('config-set', '更新后端地址，无需重装 hooks')
  ...
  .action(async (options) => run(async () => {
    ...
    if (options.host !== undefined) patch.host = String(options.host)
    if (options.port !== undefined && options.port !== '') patch.port = Number(options.port)
    if (options.protocol !== undefined) patch.protocol = String(options.protocol)
