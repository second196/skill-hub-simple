...
describe('isOurObserverHook', () => {
  ...
  it('recognizes npm package name and stable launcher', () => {
    ...
    assert.equal(
      isOurObserverHook(String.raw`C:/Users/Administrator/AppData/Local/SkillHub/observability/bin/hook.sh pre codex`),
      true
    )
  })
...
describe('rewriteEventHooks', () => {
  ...
  it('matches commandWindows ownership too', () => {
    ...
    const next = rewriteEventHooks(
      [{ hooks: [{ type: 'command', command: 'other', commandWindows: ownedWindows }] }],
      [{ command: `${LAUNCHER} pre codex`, matcher: '*' }]
    )
