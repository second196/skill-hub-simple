...
export async function scanSessionFile(clientName: ClientName, filePath: string): Promise<ObservationEvent[]> {
  const clientId = await loadClientId()
...
export async function listSessionSources(options: ScanOptions = {}): Promise<SessionSource[]> {
  ...
  for (const file of claudeFiles) {
    ...
    if (!info) continue
    if (options.sessionId && !pathLikelyMatches(options.sessionId, file, 'claude-code')) continue
    const sessionId = await claudeSessionIdFromFile(file)
    if (options.sessionId && !sessionMatches(options.sessionId, sessionId, file, 'claude-code')) continue
  ...
  for (const file of codexFiles) {
    ...
    if (options.sessionId && !pathLikelyMatches(options.sessionId, file, 'codex')) {
      ...
      if (!sessionMatches(options.sessionId, peeked, file, 'codex')) continue
    ...
    if (options.sessionId && !sessionMatches(options.sessionId, sessionId, file, 'codex')) continue
    sources.push({ clientName: 'codex', sessionId, path: file, mtimeMs: info.mtimeMs })
  }
  return sources
}

export async function findSessionSource(
  clientName: ClientName,
  sessionId: string,
  hintPath?: string,
  knownSources?: SessionSource[]
...
function pathLikelyMatches(wanted: string, file: string, clientName: ClientName): boolean {
  if (!wanted) return true
  if (sessionMatches(wanted, '', file, clientName)) return true
  if (sessionMatches(wanted, basename(file, '.jsonl'), file, clientName)) return true
  return false
}
...
function claudeSessionId(file: string, entries: Record<string, unknown>[]): string {
  ...
  if (index >= 0) {
    ...
    if (parent && child) return `${parent}/subagents/${child}`
  }
...
async function claudeSessionIdFromFile(file: string): Promise<string> {
...
async function codexSessionIdFromFile(file: string): Promise<string> {
  const id = await peekCodexSessionId(file)
  return id || basename(file, '.jsonl')
}

async function peekFirstJsonlSessionId(file: string): Promise<string> {
...
async function peekCodexSessionId(file: string): Promise<string> {
...
async function peekJsonlLines(file: string, maxLines: number): Promise<Record<string, unknown>[]> {
  const rows: Record<string, unknown>[] = []
  let handle: ReturnType<typeof createReadStream> | undefined
  try {
...
export function sessionMatches(wanted: string, found: string, file: string, clientName: ClientName): boolean {
  if (!wanted) return true
  const normalizedWanted = normalizePath(wanted)
  const normalizedFile = normalizePath(file)
  if (found && found === wanted) return true
  if (normalizedFile === normalizedWanted) return true
  ...
  if (fileBase === `${wanted}.jsonl` || (wantedBase.endsWith('.jsonl') && fileBase === wantedBase)) return true
  return false
}
...
function claudeLogicalIdFromPath(file: string): string {
  const normalized = normalizePath(file)
  const marker = '/subagents/'
  const index = normalized.toLowerCase().lastIndexOf(marker)
  if (index < 0) return ''
