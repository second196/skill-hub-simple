...
import {
  ...
  parseSkillFile,
  resolvePackageVersionFromDir,
  resolveSkillVersion,
  versionLabelFromPackageContent,
  withCompositeParents
} from './catalog.js'
...
import type { ObservationEvent, InstalledSkill } from './types.js'
...
function skillEvent(overrides: Partial<ObservationEvent> = {}): ObservationEvent {
  return {
    v: 1,
    event_id: 'e1',
    client_id: 'c1',
...
test('toIngestStep forwards version label only and strips digest fields', () => {
  ...
  const payload = ingestStep.payload as Record<string, unknown>
...
test('annotateSessions keeps local skill identity when not on platform', () => {
  ...
  const sessions = annotateSessions(
    buildTimeline([localSkill, toolStep, unversionedPlatform, unpublishedVersion]),
    [{ slug: 'using-product-development', name: 'using-product-development', versionLabels: ['1.0.0'] }]
  )
...
test('resolvePlatformSlug maps platform and composite parents, leaves local-only undefined', () => {
  ...
  assert.equal(
    resolvePlatformSlug(index, 'brainstorming', undefined, undefined, '/x/.agents/skills/superpowers/skills/brainstorming/SKILL.md'),
    'superpowers'
  )
