...
export function isInvalidVersionLabel(value: unknown): boolean {
  if (value === undefined || value === null) return true
  if (typeof value !== 'string') return true
  const trimmed = value.trim()
  if (trimmed.length === 0) return true
  if (trimmed.toLowerCase() === 'latest') return true
...
export function parseSemver(value: string): SemverParts | null {
  ...
  if (normalized.length === 0 || normalized.toLowerCase() === 'latest') return null
...
export function normalizeVersionLabel(value: string): string | null {
...
export function isGreaterThan(candidate: string, baseline: string): boolean {
 ...
 */
export function latestFormalVersion(labels: Iterable<string>): string | null {
  let latest: string | null = null
  for (const label of labels) {
    const normalized = normalizeVersionLabel(label)
    if (normalized === null) continue
    if (isPrereleaseVersion(normalized)) continue
