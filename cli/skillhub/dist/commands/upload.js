...
export async function uploadCommand(options) {
    ...
    for (const inputPath of options.inputPaths) {
        try {
            if (options.check !== false) {
                ...
                if (!inspection.ok) {
                    throw new CliError(inspection.messages[0] || 'Skill 源目录元数据不完整，拒绝上传', 'SOURCE_METADATA_INCOMPLETE', 5, {
            ...
            const sourceVerify = await verifySourcePaths([inputPath, options.sourceDir].filter(Boolean));
    ...
    for (const item of failed) {
        ...
        const sourceVerify = item.sourceVerify;
        const platformVerify = item.platformVerify;
        ...
        if (sourceVerify && sourceVerify.ok === false) {
            ...
            for (const pathResult of sourceVerify.paths || []) {
                if (!pathResult.ok)
                    lines.push(`    - ${pathResult.path}: ${String(pathResult.message || '不完整')}`);
            }
...
export async function verifySourcePaths(sourcePaths) {
...
async function verifyOneSource(sourcePath) {
...
export async function prepareCommand(options) {
    ...
    if (options.completeFrom) {
        ...
        if (!fromInspection.ok) {
            throw new CliError(fromInspection.messages[0] || '临时目录中的 Skill 包不完整，不能覆盖回源', 'SOURCE_METADATA_INCOMPLETE', 5, {
                completeFrom,
    ...
    if (!inspection.ok) {
        throw new CliError(inspection.messages[0] || 'Skill 源目录元数据不完整，无法 prepare', 'SOURCE_METADATA_INCOMPLETE', 5, {
            missing: inspection.missing,
            prepareFlow: WRITE_BACK_FLOW
        });
    }
    ...
    try {
        ...
        if (!tempInspection.ok || !tempInspection.metadata?.version) {
            throw new CliError('临时目录中的 Skill 包仍不完整', 'SOURCE_METADATA_INCOMPLETE', 5, { messages: tempInspection.messages, missing: tempInspection.missing, prepareFlow: WRITE_BACK_FLOW });
        }
        await rm(source, { recursive: true, force: true });
        await mkdir(source, { recursive: true });
...
export async function verifySourceCommand(options) {
