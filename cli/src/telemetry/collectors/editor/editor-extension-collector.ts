...
const EVENT_TYPES: Record<string, { eventType: string; tool: string }> = {
  extension_probe: { eventType: 'EDITOR_EXTENSION_PROBE', tool: 'EditorExtension' },
  extension_activated: { eventType: 'EDITOR_EXTENSION_ACTIVATED', tool: 'EditorExtension' },
  extension_deactivated: { eventType: 'EDITOR_EXTENSION_DEACTIVATED', tool: 'EditorExtension' },
  document_changed: { eventType: 'EDITOR_DOCUMENT_CHANGED', tool: 'Editor' },
  terminal_opened: { eventType: 'EDITOR_TERMINAL_OPENED', tool: 'Terminal' },
  terminal_closed: { eventType: 'EDITOR_TERMINAL_CLOSED', tool: 'Terminal' },
  task_started: { eventType: 'EDITOR_TASK_STARTED', tool: 'Task' },
  task_ended: { eventType: 'EDITOR_TASK_ENDED', tool: 'Task' },
  file_edit: { eventType: 'EDITOR_FILE_EDIT', tool: 'FileEdit' },
  terminal: { eventType: 'EDITOR_TERMINAL_COMPLETED', tool: 'Terminal' }
}
...
export class EditorExtensionCollector implements RuntimeCollector {
  ...
  collect(input: unknown, context: RuntimeCollectorContext): CanonicalRuntimeEvent[] {
    ...
    const eventDefinition = sourceType === undefined ? undefined : EVENT_TYPES[sourceType]
    const sessionId = firstIdentifier(record, ['sessionId', 'session_id'])
    if (sourceType === undefined || eventDefinition === undefined || sessionId === undefined) {
    ...
    return [finalizeCollectorEvent({
      schemaVersion: '1.0', eventId: firstIdentifier(record, ['eventId', 'event_id']),
      ...
      privacyActions: privacyActionsFor(record,
        ['relativePath', 'filePath', 'changes', 'commandLine', 'terminalOutput', 'cwd', 'prompt', 'code',
          'fileName', 'windowTitle', 'workspaceName', 'terminalName', 'taskName'])
    })]
  }
}
