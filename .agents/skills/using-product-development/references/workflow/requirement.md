...
把原始需求整理成已确认、可验收、可追溯的 `requirement.md`，为方案阶段提供稳定输入。
...
1. 检查最小相关仓库上下文和已有 Feature 文档；
2. 判断是否包含多个独立子系统，必要时先拆分；
...
原子需求清单每项使用稳定 ID，例如 `REQ-001`，并记录来源、行为、验收条件、边界和确认状态。
...
用户至少确认需求口径和写入意愿。未确认前只能输出草稿，不得写入正式 `requirement.md`。
...
同时初始化或更新控制目录中的 `state.md` 的需求状态、版本、决策、未决问题和范围，并初始化 `change-log.md`。不得写 `design.md` 或 `implementation-plan.md`。

## 7. 出口检查

写入后检查：

1. 每个功能点、流程、规则、异常和边界都有需求 ID；
...
此时必须先输出 Feature 拆分、原子需求和待确认问题；每个 `REQ-*` 只表达一个可验证行为。只有用户确认拆分和口径后，才写正式需求文档。
...
如果存在外部任务号，应与需求含义组合，例如 `feature-order-export-WEBCAPP-10234`。如果当前描述不足以提炼对象和动作，先向用户询问，不创建目录，也不使用 `REQ-*` 或纯数字临时占位。
